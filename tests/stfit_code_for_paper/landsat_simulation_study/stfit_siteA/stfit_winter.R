...
fidx = fidx4
fmat = mat0[fidx, ]
if(!dir.exists("stfit_winter"))
  dir.create("stfit_winter")
...
opts_stfit$set(temporal_mean_est = customfun)
...
res = foreach(n = 1:(M*N)) %do% {
  ...
  if(file.exists(paste0("./stfit_winter/stfit_winter_P", pidx[i], "_F", fidx[j], ".rds"))){
      tryresult <-try(res1 <- readRDS(paste0("./stfit_winter/stfit_winter_P", pidx[i], "_F", fidx[j], ".rds")))
      if(inherits(tryresult, "try-error")){
        cat('Have problem in loading', paste0("./stfit_winter/stfit_winter_P", pidx[i], "_F", fidx[j], ".rds"), 
            '; Recalculating...\n')
        res1 <- stfit_landsat(year, doy, mat, 31, 31, nnr=30, clipRange= c(0,3000),
                              use.intermediate.result = FALSE, intermediate.save = FALSE)
        saveRDS(res1, paste0("./stfit_winter/stfit_winter_P", pidx[i], "_F", fidx[j], ".rds"))
      }
  } else {
      res1 <- stfit_landsat(year, doy, mat, 31, 31, nnr=30, clipRange= c(0,3000),
                              use.intermediate.result = FALSE, intermediate.save = FALSE)
...
saveRDS(res, "./stfit_winter/res.rds")
