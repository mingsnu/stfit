...
krig_rmse_A = readRDS("./landsat_simulation_study/kriging_siteA/output/res.rds")[[1]]
...
krig_rmse_B = readRDS("./landsat_simulation_study/kriging_siteB/output/res.rds")[[1]]
...
colnames(RMSEmat) = rep(c('stfit', 'gapfill', 'kriging'),5)
RMSEmat
## for latex output
...
RE_31 = krig_rmse_A/stfit_rmse_A
...
colnames(RE) = rep(c("Spring", "Summer", "Fall", "Winter"), each=2)
RE
...
RE_31 = krig_rmse_B/stfit_rmse_B
#### RE matrix for gapfill
RE2 = matrix(NA, 3, 4)
for(i in 1:3){
  for(j in 1:4){
