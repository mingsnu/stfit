...
##########################################################
idx = doy >=100 & doy <=200
res1 = gapfill(year[idx], doy[idx], mat[idx,], 31,31, h = 0, doyrange = 100:200, nnr=5)
...
##### clustering analysis #####
###############################
cmat = values(t(ssmean[[seq(100, 200, by = 10)]]))
...
###########################################################
idx = doy >=100 & doy <=200
res2 = gapfill(year[idx], doy[idx], mat[idx,], 31,31, h = 0, doyrange = 100:200, nnr=5, cluster = cres$cluster)
...
for(k in 1:30){
  ...
  ## imputed images
  for(i in 1:n){
    r.list[[i+2*n]] = raster(matrix(res$imputed.mat[which(res$idx$idx.partialmissing == fidx[i]),], 31))
  }
  s1 = stack(r.list)
