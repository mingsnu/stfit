...
res = foreach(n = 1:(M*N)) %dopar% {
  ...
  if(file.exists(paste0("./gapfill_summer/gapfill_summer_P", i, "_F_", j, ".rds"))){
    res1 <- readRDS(paste0("./gapfill_summer/gapfill_summer_P", i, "_F_", j, ".rds"))
  } else {
    res1 = gapfill::Gapfill(tmpmat, clipRange = c(0, 3000), dopar = TRUE)
    saveRDS(res1, paste0("./gapfill_summer/gapfill_summer_P", i, "_F_", j, ".rds"))
  }
