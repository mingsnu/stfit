...
#### including kriging
##################
RMSE_krig = readRDS("./site2_kriging/output/res.rds")[[1]]
## Spring table ===================================
res = readRDS("./site2_stfit/stfit_spring/res.rds")
RMSEmat1 = matrix(unlist(lapply(res, function(x)x[1])), 15)
rownames(RMSEmat1) = paste0("P", 1:15)
colnames(RMSEmat1) = paste0("F", 1:5)
res = readRDS("./site2_gapfill/gapfill_spring/res.rds")
...
## Summer table ===================================
res = readRDS("./site2_stfit/stfit_summer/res.rds")
RMSEmat1 = matrix(unlist(lapply(res, function(x)x[1])), 15)
rownames(RMSEmat1) = paste0("P", 1:15)
colnames(RMSEmat1) = paste0("F", 6:10)
res = readRDS("./site2_gapfill/gapfill_summer/res.rds")
RMSEmat2 = matrix(unlist(lapply(res, function(x)x[1])), 15)
...
## Fall table ===================================
res = readRDS("./site2_stfit/stfit_fall/res.rds")
RMSEmat1 = matrix(unlist(lapply(res, function(x)x[1])), 15)
rownames(RMSEmat1) = paste0("P", 1:15)
colnames(RMSEmat1) = paste0("F", 11:15)
res = readRDS("./site2_gapfill/gapfill_fall/res.rds")
RMSEmat2 = matrix(unlist(lapply(res, function(x)x[1])), 15)
...
## Winter table ===================================
res = readRDS("./site2_stfit/stfit_winter/res.rds")
RMSEmat1 = matrix(unlist(lapply(res, function(x)x[1])), 15)
rownames(RMSEmat1) = paste0("P", 1:15)
colnames(RMSEmat1) = paste0("F", 16:19)
res = readRDS("./site2_gapfill/gapfill_winter/res.rds")
RMSEmat2 = matrix(unlist(lapply(res, function(x)x[1])), 15)
RMSEmat3 = RMSE_krig[,16:19]

## combine
RMSEmat = matrix(0, 15, 12)
RMSEmat[, seq(1, 12, 3)] = RMSEmat1
RMSEmat[, seq(2, 12, 3)] = RMSEmat2
RMSEmat[, seq(3, 12, 3)] = RMSEmat3
RMSEmat
rownames(RMSEmat) = paste0("P", 1:15)
colnames(RMSEmat) = paste0("F", rep(16:19, each=3))
RMSExtable = xtable::xtable(RMSEmat)

boldmat = matrix(TRUE, 15, 12)
RMSEmat2[is.na(RMSEmat2)] = 999999
boldmat[, seq(1, 12, 3)] = (RMSEmat1 < RMSEmat2) & (RMSEmat1 < RMSEmat3)
...
RMSEmat3 = readRDS("./site2_kriging/output/res.rds")[[1]]
...
for(i in 1:3){
  for(j in 1:4){
    if(j < 4){
      ...
      tmpvec = c(RMSEmat1[seq((i-1)*5+1, i*5), 16:19])
...
for(i in 1:3){
  for(j in 1:4){
    if(j < 4){
      ...
      tmpvec = c(RMSEmat2[seq((i-1)*5+1, i*5), 16:19])
      weight = rep(n.na[seq((i-1)*5+1, i*5)], 4)
      nonna.idx = !is.na(tmpvec)
      RMSEmat2.int[i,j] = sum(tmpvec[nonna.idx]*weight[nonna.idx])/sum(weight[nonna.idx])
    }
...
for(i in 1:3){
  for(j in 1:4){
    if(j < 4){
      ...
      tmpvec = c(RMSEmat3[seq((i-1)*5+1, i*5), 16:19])
      weight = rep(n.na[seq((i-1)*5+1, i*5)], 4)
      nonna.idx = !is.na(tmpvec)
      RMSEmat3.int[i,j] = sum(tmpvec[nonna.idx]*weight[nonna.idx])/sum(weight[nonna.idx])
    }
...
for(i in 1:3){
  for(j in 1:4){
    if(j < 4){
      RE2[i,j] = mean(RE_21[seq((i-1)*5+1, i*5), seq((j-1)*5+1, j*5)])
    } else {
      RE2[i,j] = mean(RE_21[seq((i-1)*5+1, i*5), 16:19])
    }
  }
}
RE2
...
for(i in 1:3){
  for(j in 1:4){
    if(j < 4){
      RE3[i,j] = mean(RE_31[seq((i-1)*5+1, i*5), seq((j-1)*5+1, j*5)])
    } else {
      RE3[i,j] = mean(RE_31[seq((i-1)*5+1, i*5), 16:19])
    }
  }
}
RE3
