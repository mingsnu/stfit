...
res = foreach(n = 1:(M*N)) %dopar% {
  ...
  if(file.exists(paste0("./gapfill_spring/gapfill_spring_P", pidx[i], "_F_", fidx[j], ".rds"))){
    res1 <- readRDS(paste0("./gapfill_spring/gapfill_spring_P", pidx[i], "_F_", fidx[j], ".rds"))
  } else {
    res1 = gapfill::Gapfill(tmpmat, clipRange = c(0, 1800), dopar = TRUE)
    saveRDS(res1, paste0("./gapfill_spring/gapfill_spring_P", pidx[i], "_F_", fidx[j], ".rds"))
  }
