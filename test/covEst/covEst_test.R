...
#### problem data set test;
#### there was some 'standalone' point problem before version 0.90.0.
mat = readRDS("./seff_bug_mat.rds")
msk = getMask(mat)
levelplot(raster(matrix(msk, 30)))
pidx = (1:ncol(mat))[!msk]
tmp = sparse_lc_cov_est1(mat[,!msk], weightMatrix(3), 30, 30, 3, pidx-1)
stmp = sparseMatrix(tmp$ridx, tmp$cidx, x = tmp$value, 
                    dims = c(sum(!msk), sum(!msk)), symmetric = TRUE)
image(stmp)
## there are missing values on diagnal
Matrix::diag(stmp)

lc_cov1_(mat[,!msk], weightMatrix(3), 0, 0, 30, 30, pidx-1)
lc_cov1_(mat[,!msk], weightMatrix(3), 44, 44, 30, 30, pidx-1)
sum((mat[,!msk][,45])^2, na.rm = TRUE)/(sum(!is.na(mat[,!msk][,45]))-1)
var(mat[,!msk][,45], na.rm = TRUE)
